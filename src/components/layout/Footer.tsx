...
const Footer: React.FC = () => {
  const navItems = [
    ...
    { name: "Services", link: "/services", icon: <IconBarbell className="h-5 w-5" /> },
    { name: "Events", link: "/events", icon: <IconCalendar className="h-5 w-5" /> },
    { name: "About", link: "/about", icon: <IconUser className="h-5 w-5" /> },
    { name: "Contact", link: "/contact", icon: <IconPhone className="h-5 w-5" /> },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="space-y-3 font-poppins">
              <div className="flex items-center space-x-3 text-black-1000 ">
                <IconMapPin className="h-5 w-5 " />
                <span>Wello sefer, Ethio China St, Addis Ababa</span>
              </div>
              <div className="flex items-center space-x-3 text-black-1000 ">
                <IconPhone className="h-5 w-5 " />
                <span>+251904242222</span>
              </div>
            </div>
          </div>
          ...
          {/* Quick Links */}
          <div>
            <h3 className="font-industry text-xl uppercase mb-6 mt-5 text-black-1000 font-bold">Quick Links</h3>
            <ul className="space-y-3 font-poppins">
              {navItems.map((item, index) => (
                <li key={index}>
                  <a
                    href={item.link}
                    className="flex items-center space-x-2 text-black-700 hover:text-[#fab313] transition-colors "
                  >
                    {item.icon}
                    <span>{item.name}</span>
                  </a>
          ...
          <div>
            ...
            <div className="flex space-x-4 mt-5 ml-13">
              ...
              <a href="https://vm.tiktok.com/ZMHnKdc5qF2eT-RqFic/" target="_blank" rel="noopener noreferrer" className="hover:opacity-80 transition-opacity" aria-label="TikTok">
                <IconBrandTiktok className="h-12 w-12 text-[#FF0050]" />
              </a>
              <a href="https://www.linkedin.com/company/afroheat/" target="_blank" rel="noopener noreferrer" className="hover:opacity-80 transition-opacity" aria-label="LinkedIn">
                <IconBrandLinkedin className="h-12 w-12 text-[#0077B5]" />
              </a>
            </div>
          </div>
        </div>
